...
using A4L.Mapprime3DNet.View.Groups;
...
public class CustomCameraController : InputHandlerBase
{
    public class KeySet : InputKeysBase
    {
        ...
        {
            get => KeyArr[_keyRight];
            set => KeyArr[_keyRight] = value;
        }

        public KeyBinding KeyFront
        {
            get => KeyArr[_keyFront];
            set => KeyArr[_keyFront] = value;
        }

        public KeyBinding KeyBack
        {
            get => KeyArr[_keyBack];
            set => KeyArr[_keyBack] = value;
        }

        public KeyBinding KeyUp
        {
            get => KeyArr[_keyUp];
            set => KeyArr[_keyUp] = value;
        }

        public KeyBinding KeyDown
        {
            get => KeyArr[_keyDown];
            set => KeyArr[_keyDown] = value;
        }

        public KeyBinding FirstPersionRotation
        {
            get => KeyArr[_firstPersionRotation];
            set => KeyArr[_firstPersionRotation] = value;
        }

        public KeyBinding ScreenCenterRotationDrag
        {
            get => KeyArr[_screenCenterRotationdrag];
            set => KeyArr[_screenCenterRotationdrag] = value;
        }

        public KeyBinding MousePointCenterRotationDrag
        {
            get => KeyArr[_mousepointCenterRotationDrag];
            set => KeyArr[_mousepointCenterRotationDrag] = value;
        }

        public KeyBinding ScreenPointCenterRotationDrag
        {
            get => KeyArr[_screenpointCenterRotationdrag];
            set => KeyArr[_screenpointCenterRotationdrag] = value;
        }

        public KeyBinding NormalZoom
        {
            get => KeyArr[_normalZoom];
            set => KeyArr[_normalZoom] = value;
        }

        public KeyBinding LessZoom
        {
            get => KeyArr[_lessZoom];
            set => KeyArr[_lessZoom] = value;
        }

        public KeyBinding GreaterZoom
        {
            get => KeyArr[_greaterZoom];
            set => KeyArr[_greaterZoom] = value;
        }
    }
    ...
    public Action<MouseEventArgs, HandleContext>? SceneMouseMoveNoButton { get; set; }
    // 기능: 대기 상태 좌클릭의 실제 장면 컨텍스트를 파노라마 클릭 피킹 경로로 전달한다.
    public Action<MouseEventArgs, HandleContext>? SceneLeftClick { get; set; }
    public Action<HandleContext>? SceneViewChanged { get; set; }
    public Action<Vector3d>? SceneViewDirectionChanged { get; set; }
    ...
    private HandleContext? _lastViewChangeContext;
    private Vector3d? _lastSceneViewDirection;
    private Vector3d? _lastSceneLookAt;
   ...
   private AnimatorBase _animatorSrs;
    ...
    // 휠 1틱당 커서 지점까지 남은 거리의 이동 비율. 거리 비례(지수) 스텝이라 어느 축척에서도 줌 속도가 동일하게 느껴진다.
    public double ZoomPercentPerTick = 0.15;

    // true면 커서 아래 픽킹 지점을 향해 줌(zoom to cursor), false면 화면 중앙(뷰 방향)으로 줌.
    ...
    public CustomCameraController()
    {
        ...
        // 기능: 기본 좌클릭을 카메라 이동과 분리해 파노라마의 가장 가까운 이미지 검색에도 전달한다.
        ((InputHandlerBase)this).LClick = new PointInputOperation();
        ((InputHandlerBase)this).LClick.ClickFunc = onLeftClick;
    ...
    public override void Render(RenderContext context)
    {
        ...
        if (_panningStarted || _bRotationStart)
        {
            if (DragLineVisible)
            {
                var size = 8;
                var x = _panningStarted ? _downPanningPoint.X : _downRotationPoint.X;
                var y = context.View.Height - 1 - (_panningStarted ? _downPanningPoint.Y : _downRotationPoint.Y);

                var rs = SelectiveRenderState.LineWidth2RenderState;

                var srs = rs.Clone();
                srs.EnableLighting(false);
                srs.EnableAlphaBlending(true);
                var curX = _panningStarted ? _dragPanningPoint.X : _preRotationDragPoint.X;
                var curY = context.View.Height - 1 - (_panningStarted ? _dragPanningPoint.Y : _preRotationDragPoint.Y);

                var cur = new Vector3d(curX, curY, 0);
                var start = new Vector3d(x, y, 0);

                var dist = (float)cur.GetDistance(start);
                var mt = new MatrixTransform();
                mt.SetRotation(Quaterniond.GetRotate(Vector3d.ZAxis, (cur - start).GetNormalized()));
                mt.SetTranslation(new Vector3d(x, y, 0));

                var gpLine = GeometryFactory.FromCylinder(0, 0, dist * 0.5f, DragLinePixelSize / 2, dist,
                    new PolygonCreationOption
                    {
                        Transform = mt,
                        Material = Material.GetColorMaterial(DragLineColor.Clone(0.125f)),
                        SelectiveRenderState = srs
                    });
                gpLine.CalculateBoundingBox();
                gpLine.RenderHintType = PrimitiveBase.RenderHintTypes.OrthogonalHud;
                context.Renderer.Add(gpLine);
            }
        }
    }
    ...
    {
        if (KeyEventArgs.IsKeyDownAlt())
            return;

        SceneMouseMoveNoButton?.Invoke(e, context);
    }

    private void onLeftClick(MouseEventArgs e, HandleContext context)
    {
        // 기능: 메인 장면 좌클릭을 구독자에게 넘겨 클릭 좌표 기반 파노라마 검색을 시작한다.
        SceneLeftClick?.Invoke(e, context);
    }

    private void onMiddleDragUp(MouseEventArgs arg1, HandleContext arg2)
    {
        onRotationDragUp(arg1, arg2);
    }
    ...
    private void onMiddleDrag(MouseEventArgs arg1, HandleContext context)
    {
        onRotationDrag(arg1, context);
    }
    ...
    public override void PreRender(RenderContext rc)
    {
            ...
            (_updatedMouseX != _currentMouseX || _updatedMouseY != _currentMouseY))
        {
            ...
            _updatedMouseX = (int)Math.Round(_updatedCurrentMouseX * (1 - ratio) + _currentMouseX * ratio);
            _updatedMouseY = (int)Math.Round(_updatedCurrentMouseY * (1 - ratio) + _currentMouseY * ratio);
            _updatedMouseX = _currentMouseX;
            _updatedMouseY = _currentMouseY;
            if (_bRotationStart)
            {
                onUpdateRotation(_updatedMouseX, _updatedMouseY, rc);
                NotifySceneViewChanged(_lastViewChangeContext);
            }
        }

        NotifySceneViewDirectionChanged(rc);
    }

    private void NotifySceneViewChanged(HandleContext? context)
    {
        if (context == null)
            return;

        SceneViewChanged?.Invoke(context);
    }

    private void NotifySceneViewDirectionChanged(RenderContext rc)
    {
        if (SceneViewDirectionChanged == null || rc.View?.Camera == null)
            return;

        var viewDirection = rc.View.Camera.GetViewVector();
        if (viewDirection == null || viewDirection.GetLength() <= 0.0001)
            return;

        viewDirection = viewDirection.GetNormalized();
        var lookAt = rc.View.Camera.LookAt;
        var lastLookAt = _lastSceneLookAt;
        bool directionChanged = _lastSceneViewDirection == null ||
            (viewDirection - _lastSceneViewDirection).GetLength() >= 0.0005;
        bool lookAtChanged = lookAt != null &&
            (lastLookAt == null || (lookAt - lastLookAt).GetLength() >= 0.05);

        if (!directionChanged && !lookAtChanged)
            return;

        _lastSceneViewDirection = viewDirection.Clone();
        _lastSceneLookAt = lookAt?.Clone();
        SceneViewDirectionChanged(viewDirection);
    }
    ...
    private void updateDrag(int mx, int my)
    {
        _currentMouseX = mx;
        _currentMouseY = my;
        _updatedCurrentMouseX = _updatedMouseX;
        _updatedCurrentMouseY = _updatedMouseY;
    ...
    private void extendLookAtWithoutEyeMove(Camera.Camera cam, Vector3d point)
    {
        ...
        if (!Plane.GetIntersectPointNormal(r, point, cam.GetViewVector(), out newLookAt))
            return;
        ...
        log("eye:",eye0,"lookat:" , cam.LookAt ,"distance:", cam.Distance);
    }

    internal void onPointMoveRotation(MouseEventArgs e, HandleContext context)
    {
        Vector3d ip = null;
        Vector3d ipNormal = null;

        var r = context.View.GetRayFromScreenCoord(e.X, e.Y);
        if (!pickTrans(r, context, out ip, out ipNormal))
            return;
        if (ip == null)
            return;
        ...
        Vector3d right = null;
        if (MathUtil.IsSameValue(ipNormal.Z, 1))
            right = (context.View.Camera.GetViewVector() ^ ipNormal.GetNegateNomalized()).GetNormalized();
        else
            right = (Vector3d.ZAxis ^ ipNormal.GetNegateNomalized()).GetNormalized();


        var up = (ipNormal.GetNegateNomalized() ^ right).GetNormalized();

        q = MathUtil.ComputeTransform(Vector3d.ZAxisNegative, ipNormal.GetNegateNomalized(), Vector3d.YAxis, up);

        dist = context.View.Camera.GetEye().GetDistance(ip) * 0.5;


        var ani = new KeyFrameCameraAnimator();
        ani.QuatInterpolationType = AnimatorBase.InterpolationEqautions.SinHalfPi;
        ani.Add(0, context.View.Camera);
        ani.Add(0.5, ip, q, dist);
        ani.Start();
        setAnimator("point", context, ani);

        updatePointer(ip, ipNormal, true);
    ...
    private bool pickTrans(Ray ray, HandleContext context, out Vector3d ip, out Vector3d outNormal)
    {
        ...
        GetPickPoints(ray, context, false, ExcludePickFuncWorld, out infos);
        ...
        if (infos.Count > 0)
        {
            ...
            ip = first.PickedPoint;
            outNormal = first.PickedNormal;
            if (outNormal == null) outNormal = context.View.Camera.GetViewVector().GetNegated();

            bPick = first.PickType != PickedIntersection.PickedTypes.Plane;
        }
    ...
    internal void onPanningDrag(MouseEventArgs e, HandleContext context)
    {
        var mx = e.X;
        var my = e.Y;
        if (!_panningStarted || _downPanningIp == null)
            return;
        ...
        if (DragSnap)
        {
            ...
            if (dist < DragSnapPixelSize)
            {
                mx = _downPanningPoint.X;
                my = _downPanningPoint.Y;
            }
        }

        // drag down 시점의 카메라 기준 ray
        var ray = context.View.GetRayFromScreenCoord(mx, my, _downPanningRot,
                                           _downPanningEye, context.View.Camera.Distance);

        var viewDir = (_downPanningLookAt - _downPanningEye).GetNormalized();
        var worldUp = new Vector3d(0, 0, 1);
        Vector3d planeNormal = worldUp;

        Vector3d ip = null;
        if (!Plane.GetIntersectPointNormal(ray, _downPanningIp, planeNormal, out ip))
        {
            Plane.GetIntersectPointNormal(ray, _downPanningIp,
            viewDir.GetNegated(), out ip);
        ...
        var newLookAt = _downPanningLookAt + offset;
        context.View.Camera.LookAt = newLookAt;
        NotifySceneViewChanged(context);


        //DebugHelper.GetInstance().AddSphere(_downPanningIp, 0.005,ColorF.Yellow);
        //DebugHelper.GetInstance().AddSphere(newLookAt, 0.005, ColorF.Magenta);


        _dragPanningPoint.X = mx;
        _dragPanningPoint.Y = my;
    }
    ...
    private void onPanningDragDown(MouseEventArgs e, HandleContext context)
    {
        ...
        {
            var worldUp = new Vector3d(0, 0, 1);
            if (Vector3d.Dot(ray.Direction, worldUp) > 0)
                return;
        }
    ...
    private bool pickRot(Ray ray, HandleContext context, out Vector3d ip, out Vector3d outNormal)
    {
        ...
        outNormal = null;
        PickedIntersections infos = null;
        var hasPick = GetPickPoints(ray, context, false, ExcludePickFuncWorld, out infos);
        ...
        // 실제 객체를 찍지 못한 경우 현재 LookAt을 지나는 카메라 수직 평면과 마우스 ray의 교차점을 가상 회전 중심으로 사용
        var fallbackPlane = new Plane(
            context.View.Camera.LookAt,
            context.View.Camera.GetViewVector().GetNegated());
    ...
    private void onRotationDragUp(MouseEventArgs e, HandleContext context)
    {
        onRotationDragUp(e.X, e.Y, context);
    }
    ...
    internal void onRotationDragDownFromCenterXY(int x, int y, HandleContext context)
    {
        ...
        if (RotationEnabled == false)
            return;
        if (!Rotation3 && CurretnRotationType != RotationTypes.FirsrPersionRotation)
            return;
        ...
        _lastViewChangeContext = context;
    ...
    internal void onUpdateRotation(int mx, int my, RenderContext rc)
    {
        ...
        if (DragSnap)
        {
            ...
            if (dist < DragSnapPixelSize)
            {
                mx = _downRotationPoint.X;
                my = _downRotationPoint.Y;
            }
        }
        ...
        else
        {
            ...
            var cy = CurretnRotationType == RotationTypes.MousePointCenterRotationDrag ? my : _downRotationPoint.Y;
            ...
            var downViewVector = ViewUtil.GetViewVector(_rotDown);
            ...
            var newray = view.GetRayFromScreenCoord(cx, cy, newQ, eye, dist);

            Vector3d ip2 = null;
            if (!Plane.GetIntersectPointNormal(newray, _rotDownIp, viewVector, out ip2))
                return;

            var newat = at0 + (_rotDownIp - ip2);

            var bApply = true;
            if (false && PlaneEnable && Planes != null && Planes.Count > 0)
            {
                var eye0 = view.Camera.GetEye();
                if (PlaneBoundary.Contain(eye0))
                {
                    var eye1 = ViewUtil.GetEye(newat, newQ, dist);
                    for (var i = 0; i < Planes.Count; i++)
                    {
                        var pl = Planes[i];
                        double ddd = 0;
                        Vector3d ip = null;
                        pl.IntersectPoint(new Ray(eye0, (eye1 - eye0).GetNormalized()), out ip);
                        if (ip != null)
                        {
                            var off0 = eye1 - eye0;
                            var off1 = ip - eye0;
                            if (off0 * off1 > 0)
                            {
                                var d = eye0.GetDistance(eye1);
                                var d2 = eye0.GetDistance(ip);
                                if (Math.Abs(d2) < d)
                                {
                                    bApply = false;
                                    break;
    ...
    internal void onRotationDrag(MouseEventArgs e, HandleContext context)
    {
        _lastViewChangeContext = context;
        updateDrag(e.X, e.Y);
    }

    private void log(params object[] arr)
    {
        //TraceUtil.WriteLine(string.Join(", ", arr));
    }

    internal void zoom(MouseEventArgs e, HandleContext context)
    {
        log($"[Wheel] delta={e.Delta} tick={Environment.TickCount} dist={context.View.Camera.Distance:F2}");
        ...
        // 커서 아래 지점 픽킹 (실패 시 pickTrans가 LookAt 평면 교차점으로 폴백)
        var ray = context.View.GetRayFromScreenCoord(e.X, e.Y);
        pickTrans(ray, context, out var ip, out _);
        if (ip == null)
            return;

        // 줌 기준 거리 = 커서 지점까지의 거리 → 축척(고도)에 비례한 스텝
        var targetDist = eye.GetDistance(ip);

        // 지수 스텝: 줌인은 남은 거리의 일정 %만큼 접근, 줌아웃은 그 역수만큼 후퇴
        var ticks = (double)e.Delta / 120.0;
        var factor = Math.Pow(1.0 - ZoomPercentPerTick, ticks);
        var newTargetDist = Math.Clamp(targetDist * factor, ZoomMinDistance, ZoomMaxDistance);
        var move = targetDist - newTargetDist;

        // eye를 커서 지점 방향으로 이동 (zoom to cursor)
        var moveDir = ZoomToCursor ? (ip - eye).GetNormalized() : cam.GetViewVector();
        var newEye = eye + moveDir * move;

        //var state = e.Delta > 0 ? "ZoomIn" : "ZoomOut";
        //TraceUtil.WriteLine($"[{state}] delta={e.Delta} factor={factor} newTargetDist={newTargetDist:F2} targetDist={targetDist:F2} newTargetDist={newTargetDist:F2} move={move:F2} eye={eye} newEye={newEye} moveDir={moveDir} move={move}");
        
        // 회전(Quat)은 유지한 채 LookAt/Distance만 재계산.
        // moveDir·viewVec 사영으로 화면 가장자리를 향해 줌해도 Distance가 정확히 유지된다.
        var viewVec = cam.GetViewVector();
        var newDist = Math.Max(cam.Distance - move * (moveDir * viewVec), ZoomMinDistance);

        cam.LookAt = newEye + viewVec * newDist;
    ...
    private void onPointMoveAnimation(MouseEventArgs e, HandleContext context)
    {
        ...
        if (dist >= ZoomMaxDistance)
            dist = ZoomMaxDistance;

        var newEye = eye + context.View.Camera.GetViewVector() * dist;
        ray.Origin = newEye;

        ray = context.View.GetRayFromScreenCoord(x, y, cam.Quat, newEye, dist);
    ...
    public bool ExcludePickFuncWorld(IPIckable obj)
    {
        if(obj is WorldGlobeRenderGroup)
        {
            return false;
        }

        if (obj is B3mdRenderGroup)
        {
            return false;
        }

        if (obj is B3dmObject)
        {
            return false;
        }
